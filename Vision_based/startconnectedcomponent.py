...
while True:
    ...
    if framecount % 10 == 0:   
       ...
       # max_pred = ndimage.gaussian_filter(max_pred, 1.0)
        ...
        #check if there is a large diffrance between images
        if framecount != 0:
            flowvalue = CalculateOpticalFlowValue(filteredFrame)
            print(flowvalue)
            if flowvalue > 0.3 or flowvalue < -0.3:
                centerPoints, img = getLines(prevFrame, img)
                print("bad frame ")
            else:
                centerPoints, img = getLines(filtered_pred, img)

            prevFrame = filtered_pred
        else:
            prevFrame = filtered_pred
